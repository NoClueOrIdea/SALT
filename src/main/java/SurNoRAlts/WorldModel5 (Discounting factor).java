...
public class WorldModel {
	...
	*/
	...
	public WorldModel(int statesnum, int actionsnum)
	{
		...
		alpha=new double[number_of_states][number_of_actions][number_of_nfr];
		alpha_t1=new double[number_of_states][number_of_actions][number_of_nfr];
		alpha_probs=new double[number_of_states][number_of_actions][number_of_nfr];
	...
	public void updateBelief(int cstate,int selected_action,int sprime)
	{
		...
		*/
		...
		for(int sat_NFR=0;sat_NFR<number_of_nfr;sat_NFR++)
		{
			...
			//alpha[cstate][other_action][sat_NFR]=((1-adaptation_rate)*alpha[cstate][other_action][sat_NFR])+(adaptation_rate*alpha_t1[cstate][other_action][sat_NFR])+(1-kroneckers_delta);
	...
	public void updateTransitionModel()
	{
		...
		for(int action=0;action<number_of_actions;action++)
		{
			...
			{
				//First calculate the probabilities of a given NFR-action pair leading to satisfaction
				...
				/*
				 * Transitions to a new state are calculated as follows:
				 * We take the alpha_probabilities of the current state which give the probabilities of an NFR being satisifed by taking a certain action at the current state
				 * Next we find out which NFR's are satisfied in the next state
				 * We calculate the probability of entering a new state to be the joint probabilities of the NFR's satisfactions
				 * So if all NFR's are satisifed we multiply each member of alpha_probs by one another
				 * If a state however is unsatisfied we take 1-alpha_probs (corresponding to that NFR) as this gives us the probability of the NFR NOT being satisfied
				 */
				double denominator1=0;
				...
				//This section tries to deal with two errors, the first is that the probability distributions produced via joint probability do not always sum to 1
				//the second is an issue with decimals in certain edge cases breaking the program
				double rn_sum = 0;
				for(int sprime=0;sprime<number_of_states;sprime++)
				{
